...
# default simulator (vsim, xsim or ghdl)
DEFAULTSIM	:= vsim
# default GUI mode
DEFAULTGUI	:= no
...
DEFAULTDIR	:= /tmp/$(USER)/$(PROJECT)/$(SIM)
# actual temporary working directory
DIR			?= $(DEFAULTDIR)
...
# compilation mode: work: the default target library is work, dirname: the
# default target library is the one with same name as the directory of the
# source file
DEFAULTMODE	:= work
MODE		?= $(DEFAULTMODE)
...
ifeq ($(SIM),xsim)
# no need to compile unisim with Vivado
SRC			:= $(filter-out %/unisim.vhd,$(SRC))
endif
...
Examples:
    make DIR=/tmp/mysim SIM=xsim V=1 foo_sim
    make DIR=/tmp/ghdl_sim SIM=ghdl GUI=yes foo_sim.sim

Variable  domain             description (default)
    SIM   {vsim,xsim,ghdl}   simulation toolchain ($(SIM))
...
# with Vivado unisim tag is just touched, no compilation
ifeq ($(SIM),xsim)
unisim:
	touch $@
endif
